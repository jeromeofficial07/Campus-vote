...
function AuthProvider({ children }) {
  const [user, setUser] = useState(() => {
    ...
    const maxInactivityMs = 2 * 60 * 60 * 1000; // 2 hours
    if (lastActive && Date.now() - parseInt(lastActive, 10) > maxInactivityMs) {
      sessionStorage.removeItem("token");
      sessionStorage.removeItem("user");
      sessionStorage.removeItem("last_activity");
      return null;
  ...
  // Inactivity auto-logout check (2 hours)
  useEffect(() => {
    ...
    const checkTimeout = () => {
      ...
      const token = sessionStorage.getItem("token");
      const maxInactivityMs = 2 * 60 * 60 * 1000; // 2 hours
      if (token && lastActive && Date.now() - parseInt(lastActive, 10) > maxInactivityMs) {
        logout();
      }
    };
  ...
  // Re-fetch user profile on page refresh to keep verification status fresh
  useEffect(() => {
    const token = sessionStorage.getItem("token");
    if (!token) return;
    apiFetch("/api/auth/me")
      ...
      .catch(() => {
        // If token expired, apiFetch 401 handler logs user out cleanly
      });
  }, []);
  ...
  // Join/leave user private socket room
...
function LoadingSpinner({ label = "Loading...", minHeight = "200px" }) {
  return (
    <div className="loading-center-container" style={{ minHeight }}>
      <div className="election-spinner">
        <div className="spinner-ring" />
        <div className="spinner-seal">CV</div>
      </div>
  ...
  return (
    <div className="light-page">
      <div className="container">
        ...
        {loading && <LoadingSpinner label="Loading active elections…" minHeight="240px" />}
      ...
      <div className="container" style={{ maxWidth: 880 }}>
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div className="form-section-card">
            ...
            {candidates.map((c, i) => (
              <div className="candidate-builder-card" key={i}>
  ...
  return (
    <div className="light-page">
      <div className="container">
        ...
        {loading ? <LoadingSpinner label="Loading Control Center & Audit data…" minHeight="300px" /> : (
          <div>
  ...
  return (
    <div className="light-page">
      <div className="container">
        ...
        {loading ? <LoadingSpinner label="Loading Voter Integrity & Audit Logs…" minHeight="300px" /> : (
          <div className="ballot" style={{ padding: 24, overflowX: "auto" }}>
            {tab === "roster" ? (
              <table className="audit-table">
                <thead>
...
function PollDetailPage() {
  ...
  if (!poll) {
    return (
      <div className="light-page polling-page-bg" style={{ minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center" }}>
        <LoadingSpinner label="Loading election ballot details…" minHeight="300px" />
      </div>
    );
  }
  ...
  return (
    <div className="light-page polling-page-bg">
  ...
  return (
    <div className="light-page">
      <div className="container">
        ...
        {loading ? (
          <LoadingSpinner label="Loading student verification roster…" minHeight="260px" />
        ) : filtered.length === 0 ? (
          <div className="empty-state">
